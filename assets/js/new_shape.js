...
container.addEventListener('drop', (e) => {
    ...
    if (shape_id == 'square') {
        ...
        group.on('dblclick', (ev) => {
            ...
            // console.log("Rect list ===> ", rect_list)
            group.getChildren((node) => {
                if (node.getClassName() === 'Text') {
                    // node.text(inp)
        ...
        btn_param.addEventListener('click', (e) => {
            ...
            rect_list.forEach((el) => {
                if (el.children[2].attrs.text === obj_partner.reciver) {
                    ...
                    connect_node.to.bar = el.children[0].attrs
                        // connect_node.pos_bar = el.children[0].absolutePosition()
            ...
            // arrow_distance = create_arrow(connect_node, distance_partner)

            connect_node.arrow = arrow_distance
            arrow_list.push(connect_node)
                // console.log("connect_node", connect_node)
                // console.log(" =======> ", arrow_list)
                // console.log("arrow_list => ", arrow_list.splice(-1)[0].arrow.attrs)
            ...
            note_group.absolutePosition({
                x: currentGroupName.getX - 150,
                y: currentGroupName.getY + 60
            })
            layer.add(note_group)
